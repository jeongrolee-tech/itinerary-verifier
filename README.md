...
**여기가 이 프로젝트의 핵심이다.** 휴무일도 입장도 통과했는데 전체 판정은 `feasible`이 아니다.
...
| [`label-review-260922.md`](docs/label-review-260922.md) | 라벨 재검토에 쓴 질문지 | 노후화된 8건에 대해 입력·기존 라벨·바뀐 데이터·주행시간 하한선만 적어두고 **정답 칸은 비워 둔 채로** 판단했다. **코드가 무엇을 내는지 일부러 적지 않은 것**이 요점이다. 결론은 `tests.json`에 들어갔고 재검토는 끝났다 |
...
| **T04**<br>`dev` | …17시 30분에 경복궁 들어가서 한 시간 정도 보고, 19시에 광장시장에서 저녁을 먹은 다음 20시 30분 서울역에서 KTX를 탑니다. **이 기차는 꼭 타야 해요.** | `infeasible` | 위반은 입장마감(17:00)에서 확정된다. KTX 쪽은 **열차 출발 시각(사용자)과 15분 전 도착(우리 기준)을 한 덩어리로 섞는 구현**을 잡는다 |
...
라벨 분포 — `infeasible` 6 · `feasible` 2 · `undetermined` 7.
`undetermined`가 절반인 것은 데이터가 부족해서이고, **그 부족을 감추지 않는 것이 이 프로젝트의 주장이다.**
