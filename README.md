...
| T08 | 체류시간 미입력 | 기본값으로 메워 판정 ← **`llm_with_facts` 가 여기서 누출** |
| T09 | 계절 경계 | 연중 같은 시간으로 처리 |
| T12 | 기본값이 판정을 흔듦 | 설정값으로 확정 오류 생성 |
| T10 | 계절별 시간표 정상 | 계절 3구간 중 맞는 행을 고르나 (정상 케이스) |
| T13 | 회차 입장 검사 없음 | 정보는 확보했는데 검사가 없는 상태를 정보 부족과 섞는가 |
| T14 | 근거 만료 | 조회 시각만 보고 적용 ← **`llm_naive` 가 여기서 누출** |
