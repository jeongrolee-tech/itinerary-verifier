...
| 사실 데이터 | ❌ | ❌ | ✅ | ✅ |
| "근거 없으면 보류하라" 지시 | ❌ | ✅ | ✅ | — |
| 전체 정확도 | 0.714 | 0.286 | 0.857 | **1.0** |
| 오류 표시 중 실제 오류 | 1.0 | – | 1.0 | 1.0 |
| 실제 오류 중 찾아낸 비율 | 0.75 | 0.0 | 1.0 | 1.0 |
| **미확인을 통과로 (건)** | **1** | 0 | **1** | **0** |
| 판정 보류 비율 | 0.286 | 1.0 | 0.143 | 0.286 |
| 유효 판단 제공 비율 | 0.714 | 0.0 | 0.857 | 0.714 |
| 건당 비용 | $0.0186 | $0.0166 | $0.0688 | **$0** |
| 지연 중앙값 | 9.2초 | 8.2초 | 12.1초 | **밀리초** |

**1. 지시를 안 하면 근거 없이 단정한다** (`llm_naive` → `llm_only`)

둘은 **입력이 완전히 같다.** 시스템 프롬프트만 다르다. 사실 데이터는 둘 다 받지 않았다.

```
지시 없음   보류 0.286   →  71%를 운영시간·휴무일 없이 단정했다
지시 있음   보류 1.0     →  전부 보류. 안전하지만 유효 판단 0%
```

`llm_naive`는 `facts.json`을 한 줄도 받지 않았는데 T02를 `infeasible`, T06을 `feasible`이라고 단정했다. 자기 지식으로 판단한 것이다. **1차 실험에서 `llm_only`가 전부 보류한 것은 LLM의 성질이 아니라 내 프롬프트의 결과였다.**

**2. 같은 결정성에서 코드가 더 정확하다** (`llm_naive` vs `code`)

보류 비율과 유효 판단 비율이 **0.286 / 0.714로 완전히 같다.** 판단한 양이 같다는 뜻이다. 그런데 정확도는 0.714 대 1.0이고 누출은 1건 대 0건이다.

> 코드가 "보수적이라 안전한" 게 아니다. **같은 양을 판단하면서 더 맞힌다.**

게다가 **보류하는 대상이 다르다.** 둘 다 2건을 보류했는데 겹치는 건 T08 하나뿐이다.

| 케이스 | 정답 | `llm_naive` | `code` |
| --- | --- | --- | --- |
| T08 체류시간 미입력 | `undetermined` | `undetermined` ✅ | `undetermined` ✅ |
| **T14 근거 만료** | `undetermined` | `feasible` ❌ **누출** | `undetermined` ✅ |
| **T15 공휴일 시간표** | `infeasible` | `undetermined` ❌ 놓침 | `infeasible` ✅ |

T14는 `"2027년 3월 5일 금요일 오후 1시에 경복궁"`이다. 확보한 근거가 2026년 기준이고 `valid_until`이 `2026-12-31`이라 **2027년에는 쓸 수 없다.** `llm_naive`는 그걸 `feasible`로 냈다.

**3. 정보를 주면 크게 나아지지만 누출은 남는다** (`llm_with_facts`)

정확도 0.857로 가장 많이 판단했지만(0.857) **T08에서 누출 1건**이다. `"오후 4시 반에 경복궁"` — 체류시간을 말하지 않았는데 `feasible`을 냈다. `policy.json`의 `require_user_dwell_for_completion_check: true`를 **문서로 받고도** 적용하지 않았다.

```
arm0 → 1   지시 효과        보류 0.286 → 1.0. 안전해지지만 쓸모없어진다
arm1 → 2   정보 제공 효과    정확도 0.286 → 0.857
arm2 → 3   판정 방식 효과    누출 1 → 0
```

> 표본 7건이다. 정밀도·재현율의 신뢰구간은 넓다. 다만 **누출은 "있다"를 확정하는
> 쪽이라** 표본이 작아도 유효하고, 보류 비율은 비율로 읽을 수 있다.
