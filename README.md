...
```
검사 하나            pass            근거를 확보했고 조건을 만족했다
                     fail            확보한 근거에서 위반이 확인됐다
                     unknown         검사가 필요한데 끝내지 못했다
                     not_applicable  검사 자체가 성립하지 않는다
```

`unknown`이 없으면 서비스가 거짓말을 한다. 운영시간을 못 구한 것을 "문제 없음"으로 내기
때문이다. `not_applicable`을 따로 둔 이유도 같다 — **검사가 불필요한 것과 못 끝낸 것은
다르다.** 마지막 스톱에 다음 구간이 없는 것은 미확인이 아니다.

일정 전체 판정은 이 검사들을 **가장 나쁜 상태로 접어서** 낸다. 다수결이 아니다.

```
일정 전체            infeasible      fail 이 하나라도 있다
                     undetermined    fail 은 없지만 unknown 이 있다
                     feasible        필요한 검사가 모두 통과했다
```

`not_applicable`은 전체 판정을 끌어내리지 않는다. 잘못된 것이 아니기 때문이다.
반대로 `unknown`을 무시하고 "대부분 통과했으니 `feasible`"을 내는 순간, 이 검증기는
...
| 구간 | 주행시간 하한선 | 빠진 것 |
| --- | --- | --- |
| 미술관 → 경복궁 | 6.5분 (환승 1회) | 장소→역 도보 · 환승 이동 · 열차 대기 |
| 경복궁 → 광장시장 | 4.5분 (환승 1회) | 〃 |
| 광장시장 → 서울역 | 7.0분 (환승 0회) | 〃 |

**빠진 몫이 얼마인지는 모른다.** 그 값을 알려면 실제 문전 이동시간이 필요한데,
확보 가능한 출처가 아직 없다. 하한선 판정이 성립하는 이유가 오히려 이것이다 —
크기를 몰라도 **방향은 알기 때문에** 한쪽으로만 확정할 수 있다.
...
- **도보·환승·대기 이동시간** — `undetermined`의 최대 원인이다. 지금은 주행시간 하한선만 있고, 빠진 몫이 얼마인지도 모른다
